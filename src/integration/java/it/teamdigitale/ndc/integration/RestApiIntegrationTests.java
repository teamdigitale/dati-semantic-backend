...
public class RestApiIntegrationTests {
    ...
    void shouldBeAbleToHarvestAndSearchControlledVocabularySuccessfully() throws IOException {
        ...
        searchResponse.then()
                ...
                .body("data[0].assetIri", equalTo("https://w3id.org/italia/controlled-vocabulary/licences"))
    ...
    void shouldBeAbleToHarvestAndSearchOntologieSuccessfully() throws IOException {
        ...
        getSemanticAssetDetails(getAssetIri(searchResponse)).then()
                ...
                .body("keyClasses[0].iri", equalTo("https://w3id.org/italia/onto/ACCO/AccommodationRoom"));
    }
    ...
    void shouldBeAbleToHarvestAndSearchSemanticAssetWhenFlatFileIsMissing() throws IOException {
        ...
        //when
        Response searchResponse = getSemanticAsset("Istruzione", SemanticAssetType.CONTROLLED_VOCABULARY);
    ...
    void shouldNotHarvestCorruptedControlledVocabulary() throws IOException {
        ...
        //when
        Response searchResponse = getSemanticAsset("Appellativo", SemanticAssetType.CONTROLLED_VOCABULARY);
        ...
        when().get(String.format("http://localhost:%d/agid/personTitle", port)).then().statusCode(404);
    }
    ...
    void shouldNotHarvestControlledVocabularyIfKeyConceptIsMissing() throws IOException {
        ...
        //when
        Response searchResponse = getSemanticAsset("scientifiche", SemanticAssetType.CONTROLLED_VOCABULARY);
    ...
    private Response getSemanticAsset(String searchTerm, SemanticAssetType semanticAssetType) {
        return when().get(format("http://localhost:%d/semantic-assets/search?term=%s&type=%s", port,
                searchTerm, semanticAssetType.name()));
    }

    private Response getSemanticAssetDetails(String iri) {
        return when().get(format("http://localhost:%d/semantic-assets/details?iri=%s", port, iri));
    }

    private String getAssetIri(Response searchResponse) {
        return searchResponse.getBody().as(SemanticAssetSearchResult.class).getData().get(0).getAssetIri();
    }
}
